...
def controller():
    Hello
    return send_detaiiled report
    Return "detailed report"


from message.contrib import message
from django.contrib import messages
data  = data.success(request,'Hello to request the data')

from datetime import date
date_info = date.date(sdsd)
time_info = date.time()


def controlling(request):
    pandas = "pandas.contributing the detailed message"
    message.contrib = 'hello viewers hi and hello'
    return "May I help you with concern"


def controller_unit(request):
    msg.success(request,"Enter the details and save the file")
    return "Welcome people"

def welcome_note(request):
    conducting = welcome_messages 
    print('Additional Datas for marketing datas')
    prevent_escalating datas = 
    return 'show me the data'

def greet(request):
    return (request,'hello.html')
